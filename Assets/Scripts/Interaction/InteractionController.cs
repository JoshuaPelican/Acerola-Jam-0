...
public class InteractionController : MonoBehaviour
{
    ...
    [Range(0, 1)] 
    [SerializeField] float minimumCharge = 0.25f;
    ...
    void ChargeThrow()
    {
        chargeAmount = Mathf.Clamp01(chargeAmount + Time.deltaTime * chargeRate);
    ...
    IEnumerator Pickup(Pickup pickup)
    {
        ...
        {
            held.transform.SetLocalPositionAndRotation(Vector3.Slerp(held.transform.localPosition, Vector3.zero, pickupSpeed), Quaternion.Slerp(held.transform.localRotation, Quaternion.identity, pickupSpeed));
            yield return null;
        }
    }
    ...
    {
        Transform placed = held.transform;
        held.Place();
        held = null;

        while (Vector2.Distance(new Vector2(placed.position.x, placed.position.z), new Vector2(targetPosition.x, targetPosition.z)) > 0.2f)
        {
            //Interpolate the position
            placed.position = Vector3.Slerp(placed.position, targetPosition, pickupSpeed);

            //Interpolate the rotation, resetting any crooked rotations
            Vector3 localAngles = placed.localRotation.eulerAngles;
            localAngles.x = 0;
            localAngles.z = 0;
            placed.localRotation = Quaternion.Euler(localAngles);
